...
<body>
    ...
    <div class="nav-scroller bg-warning shadow-sm">
      <nav class="nav nav-underline">
        <small><a class="nav-link" href="/match.spn?league=EPL">프리미어</a></small>
        <small><a class="nav-link" href="/match.spn?league=SPL">라리가</a></small>
        <small><a class="nav-link" href="/match.spn?league=GPL">분데스리가</a></small>
        <small><a class="nav-link" href="/match.spn?league=IPL">세리아A</a></small>
        <small><a class="nav-link" href="/match.spn?league=FPL">리그1</a></small>
        <small><a class="nav-link" href="/match.spn?league=UCL">챔피언스</a></small>
        <small><a class="nav-link" href="#">K리그</a></small>
      </nav>
    </div>
	
	<div class="my-3 p-3 bg-white rounded shadow-sm">
      <h6 class="border-bottom border-gray pb-2 mb-0 font-weight-bold text-primary">경기 일정</h6>
      <c:forEach var="match" begin="0" end="${size}" items="${match_list}" varStatus="status">
        <div class="media text-muted pt-3">        
        <div class="media-body pb-2 mb-0 lh-125 border-bottom border-gray">
          <div class="d-flex justify-content-between mb-3 small align-items-center w-100">
            <c:choose>
				<c:when test="${match.league == 'SPL' }">
					<a href="/match.spn?league=SPL" class="text-dark"><img src="/images/le_spl.png" alt="..." class="img-thumbnail"> 라리가</a>
				</c:when>
				<c:when test="${match.league == 'IPL' }">
					<a href="/match.spn?league=IPL" class="text-dark"><img src="/images/le_ipl.png" alt="..." class="img-thumbnail"> 세리에A</a>
				</c:when>
				<c:when test="${match.league == 'GPL' }">
					<a href="/match.spn?league=GPL" class="text-dark"><img src="/images/le_gpl.png" alt="..." class="img-thumbnail"> 분데스리가</a>
				</c:when>
				<c:when test="${match.league == 'FPL' }">
					<a href="/match.spn?league=FPL" class="text-dark"><img src="/images/le_fpl.png" alt="..." class="img-thumbnail"> 리그1</a>
				</c:when>
				<c:when test="${match.league == 'UCL' }">
					<a href="/match.spn?league=UCL" class="text-dark"><img src="/images/le_ucl.png" alt="..." class="img-thumbnail"> 챔피언스리그</a>
				</c:when>
				<c:otherwise>
					<a href="/match.spn?league=EPL" class="text-dark"><img src="/images/le_epl.png" alt="..." class="img-thumbnail"> 프리미어리그</a>
				</c:otherwise>
			</c:choose>
      ...
      <small class="d-block text-right mt-3">
        <a href="/result.spn">리그별 경기 결과 확인하기</a>
      </small>
    </div>	
